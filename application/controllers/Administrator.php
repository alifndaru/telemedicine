...
class Administrator extends CI_Controller
{
    ...
    function tambah_halamanbaru()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($hasil['file_name'] == '') {
                $data = array(
                    'id_kategori_halaman' => $this->db->escape_str($this->input->post('aa')),
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($hasil['file_name'] == '') {
                $data = array(
                    ...
                    'user_id' => $user_id,
                    'spesialis' => cetak($this->input->post('spesialis'))
                );
            } else {
                $data = array(
                    'username' => $this->db->escape_str($this->input->post('a')),
        ...
        if (isset($_POST['action'])) {
            ...
            if ($_POST['action'] == 'create') {
                if (
                    ...
                    $data_klinik = array(
                        'klinik' => $this->db->escape_str($this->input->post('nama_klinik')),
    ...
    function fetch_klinik()
    {
        ...
        // Assuming $ref is not used and can be safely set to null
        $ref = null; // or appropriate value based on your application's logic

        if ($level === 'klinik') {
            $result = $this->model_app->fetch_klinik_2($d);
        } else {
            $result = $this->model_app->fetch_klinik($d, $ref); // Modified to include $ref
        }
