...
var application = new Vue({
    ...
    methods: {
        ...
        fetchJadwalDokter: function() {
            axios.post('../administrator/xhrJadwalDokter', JSON.stringify({
            data: {
                ref: 'semua',
        ...
        tambahJadwal: function() {
            ...
            if (valide) {
                axios.post('../administrator/xhrJadwal', JSON.stringify({
                    data: {
                        ref: 'tambah',
        ...
        editMode: function(id, tstart, tend, kuota_pasien, timezone, status) {
            this.actAdd = false;
            this.actEdit = true;
            this.tstart = tstart;
            this.tend = tend;
        ...
        validateJadwalF: function() {
            ...
            if (this.actAdd) {
                for (var i = 0; i < this.jadwal.length; i++) {
                    var tr = [this.jadwal[i].tstart, this.jadwal[i].tend];
                    dx.push(tr);
                }
            } else {
                for (var i = 0; i < this.jadwal.length; i++) {
                    if (this.jadwal_id !== this.jadwal[i].id) {
                        var tr = [this.jadwal[i].tstart, this.jadwal[i].tend];
                        dx.push(tr);
                    } 
                }
            ...
            for (const newInterval of newIntervals) {
                if (!this.isOverlapping(dx, newInterval)) {
                    goodCount += 1
                }
            }
            ...
            if (this.tstart >= this.tend) {
                ...
                return goodCount === newIntervals.length;
            }
